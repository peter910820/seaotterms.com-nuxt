...
const form2 = ref<TodoTopicCreateRequest>({
  topicName: "",
  topicOwner: "system",
  updatedAt: new Date(),
  updateName: user.value.username,
...
const handleSubmit = async (isSystem: boolean) => {
  if (isSystem) {
    if (form2.value.topicName.trim() === "") {
      alert("標題不得為空");
      return;
    }
  ...
  <div class="row main-block">
    ...
    <div class="col s12 sub-block wow animate__flipInX">
      <div class="row">
        ...
        <div class="col s12">
          <button @click="handleSubmit(false)" class="button-submit" type="button">
            建立主題
    ...
    <div class="col s12 sub-block wow animate__flipInX">
      <div class="row">
        <div class="input-field col s12">
          <i class="material-icons prefix">title</i>
          <input id="title" v-model="form2.topicName" type="text" class="validate" required />
          <span class="helper-text" data-error="此欄不能為空" data-success=""></span>
          <label for="title">title</label>
        </div>
        <div class="col s12">
          <button @click="handleSubmit(true)" class="button-submit" type="button">
            建立系統站台
            <i class="material-icons right">send</i>
          </button>
        </div>
